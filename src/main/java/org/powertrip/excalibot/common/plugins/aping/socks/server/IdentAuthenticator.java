...
//////////////////
  private int getRangeIndex(InetAddress ip){
     int index = 0;
     Enumeration enume = hosts.elements();
     while(enume.hasMoreElements()){
       InetRange ir = (InetRange) enume.nextElement();
  ...
  private String userNames(int i){
    ...
    Enumeration enume = ((Hashtable)users.elementAt(i)).keys();
    if(!enume.hasMoreElements()) return "";
    String s = enume.nextElement().toString();
    while(enume.hasMoreElements())
       s += "; "+enume.nextElement();
