...
public class InetRange implements Cloneable{
    ...
    public synchronized boolean remove(String s){
      Enumeration enume = all.elements();
      while(enume.hasMoreElements()){
        Object[] entry = (Object[]) enume.nextElement();
        if(s.equals(entry[0])){
...
/////////////////
    ...
    private synchronized boolean contains(long ip){
       Enumeration enume = all.elements();
       while(enume.hasMoreElements()){
         Object[] obj = (Object[]) enume.nextElement();
    ...
    private boolean checkHostEnding(String host){
       Enumeration enume = end_names.elements();
       while(enume.hasMoreElements()){
          if(host.endsWith((String) enume.nextElement())) return true;
       }
       return false;
    }
